...
WORK_DIRECTORY = 'data'
IMAGE_SIZE = 28  # 图片的实际尺寸是28*28
NUM_CHANNELS = 1  # 黑白的图片只有一个channel，彩色rgb有三个channel
PIXEL_DEPTH = 255  # 每个像素值原为[0, 255]
NUM_LABELS = 10  # mnist数据集有10个类别，0-9这10个数字
...
# 事先在tf中定义好boolean变量，执行图中需要的boolean
...
def extract_data(filename, num_images):
  ...
  with gzip.open(filename) as bytestream:
    ...
    data = (data - (PIXEL_DEPTH / 2.0)) / PIXEL_DEPTH  # 每个像素值原为[0, 255]，这里转化为[-0.5, 0.5].
...
# 程序主入口
def main(argv=None):  # pylint: disable=unused-argument，pylint代码风格检查工具，提示代码风格问题
  ...
  # conv1 @(28+2*2-5)/strides(1)+1, 28*28(no change) ~~~ pool ~~~ (28-2)/strides(2)+1 , 14*14
  ...
  # conv2 @(14+2*2-5)/1+1 ~~~ 14*14(no change) ~~~ pool ~~~ 7*7
  ...
  def model(data, train=False):
    ...
    # fc1 [imageIndex, y, x, depth] ~~~ [imageIndex, y*x*depth]
  ...
  with tf.Session() as sess:
    ...
    tf.initialize_all_variables().run()
    print('mnist convolutional Initialized!')
...
if __name__ == '__main__':
    print('\n*********************** the mnist convolutional started ***********************\n')
    tf.app.run()
    print('\n*********************** the mnist convolution end ***********************\n')
