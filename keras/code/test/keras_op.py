...
def triplet_loss(y_true, y_pred):
    margin = K.constant(1)
    d = K.square(y_pred[:, 0, 0]) - 0.5 * (K.square(y_pred[:, 1, 0]) + K.square(y_pred[:, 2, 0]))
    dist = K.maximum(K.constant(0), d + margin)
    return K.mean(dist)
...
#
#     print
#     'Dataset has ' + str(total_t) + ' training triplets, and ' + str(total_v) + ' validation triplets.'
#
#     print
#     'Creating a model ...'
...
#             print
#             'Epoch ' + str(e) + ': train chunk ' + str(t + 1) + '/ ' + str(total_t_ch) + ' ...'
#
#             print
#             'Reading image lists ...'
...
#             model.fit_generator(generator=createDataGen(anchors_t, positives_t, negatives_t, Y_train, batch),
#                                 steps_per_epoch=len(Y_train) / batch, epochs=1, shuffle=False, use_multiprocessing=True)
#
...
#             print
#             'Loading validation image lists ...'
#             print
...
#
#         print
#         'Validation Results: ' + str(val_res)
#
#     print
#     'Saving model ...'
#
